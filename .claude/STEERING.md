...
| **オーケストレーション** | 明示要求時のみ（多エージェント並列） | 高（要オプトイン） | `.claude/workflows/*.js`（Workflow ツールの名前付きワークフロー） |
...
### オーケストレーション層 — `.claude/workflows/`（Workflow ツール）

多エージェントを**構造化して**使う名前付きワークフロー。トークンコストが高いため、
**ユーザーの明示要求時のみ**（「オーケストレーションして」「徹底的に」「バグゼロに」等）起動する。
手順・パターン・停止条件は `orchestrate` スキル（`.claude/skills/orchestrate/SKILL.md`）を参照。

- **`bug-hunt`**（`/bug-hunt <対象>`）… 対象領域を観点別レンズ（正確性 / 状態 / EDT /
  I・O・i18n / UX）で並列調査し、bug 判定を独立エージェントが敵対的に検証して
  **確定バグだけ**を返す。修正はメインループが直列に行い、「新規確定 0 件」まで
  ラウンドを回す（loop-until-dry）。
...
**追加候補（未適用・ユーザー判断待ち）**: セッション実績から、以下も allow に足すと
プロンプトがさらに減ります（エージェント自身による権限拡大はガードされるため、人が
`/permissions` か settings.json 直接編集で追加してください）:
`Bash(xvfb-run -a gradle *)` `Bash(xvfb-run -a ./gradlew *)` `Bash(xvfb-run -a java *)`
`Bash(javac *)` `Bash(javap *)` `Bash(git fetch *)` `Bash(git show *)`
`Bash(git merge-base *)` `Bash(head *)` `Bash(tail *)` `Bash(mkdir -p *)`
