...
* **変数・フィールドに「格納」されたコールバックをシーケンス図/アクティビティ図で可視化** (`InlineCallbacks` 新規 / `PlantUmlSequenceDiagram` / `SeqEmitters` / `SeqRender` / `PlantUmlActivityDiagram`)
    ...
    * **ローカル変数コールバックを「宣言位置」ではなく「実際の呼び出し位置」で展開**: 従来は宣言位置に本体が描かれ、実行順序が事実と異なって見えていた (両図種とも)。直接呼び出されない変数 (他メソッドへ渡すだけ等) は従来どおり宣言位置で展開して取りこぼさない。
    * **静的に解決できないコールバックは「未展開」note で可視化**: setter 経由・コレクション格納・実行時注入のリスナー等、静的解析で実体を特定できない呼び出しが無言で消えるのをやめ、`⋯ run() の実体は実行時に格納されるため未展開` の note を出す (対象は Runnable/Consumer/\*Listener 等コールバックらしき型のフィールドに限定してノイズを防止。`showUnresolvedCallbackNote` オプションで無効化可)。
    * テスト: `PlantUmlStoredCallbackTest` (16 ケース) を新設し、呼び出し位置展開・宣言位置フォールバック・note の発火/抑止/無効化・再帰打ち切り・実レンダリング (構文エラーなし) を固定。既存のインライン展開テスト群は全て非破壊。
    * 目的: 「コールバックなど関数を格納するとアクティビティ/シーケンスで処理が見えない」という利用者の不満を、静的に追える範囲は本体展開で、追えない範囲は理由の明示で解消するため。
