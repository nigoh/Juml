...
public final class CliDispatcher {
    ...
    /** Apktool 逆コンパイル APK 系モード ({@code --apk} 系) のディスパッチ。 */
    private static boolean dispatchApk(CliOptions o, CliContext ctx) throws IOException {
        if (o.apkDecode.isSet()) {
            ApkCommands.handleApkDecode(ctx);
            return true;
        }
        if (o.apk.isSet()) {
            ApkCommands.handleApkAll(ctx, apkPackagePrefix(o));
            return true;
        }
        if (o.apkSummary.isSet()) {
            ApkCommands.handleApkSummary(ctx, apkPackagePrefix(o));
            return true;
        }
        if (o.apkClassDiagram.isSet()) {
            ApkCommands.handleApkClassDiagram(ctx, apkPackagePrefix(o));
            return true;
        }
        if (o.apkSmali.isSet()) {
            ApkCommands.handleApkSmali(ctx, apkPackagePrefix(o));
            return true;
        }
        if (o.apkSequence.isSet()) {
            ApkCommands.handleApkSequence(ctx,
                    o.apkSequence.getArguments().getLast(), apkPackagePrefix(o));
            return true;
        }
        return false;
    }

    /** {@code --apk-package} の値 (未指定なら null)。 */
    private static String apkPackagePrefix(CliOptions o) {
        return o.apkPackage.getArguments().isEmpty()
                ? null : o.apkPackage.getArguments().getLast();
    }
}
