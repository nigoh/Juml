...
/**
 * ユースケース図デザイナーの編集ダイアログ (要素の id/種別/表示名・関係の種別/ラベル)。
...
final class UseCaseSketchDialogs {
     ...
     */
    static boolean editNode(Component parent, UseCaseSketchModel model, UseCaseNode target) {
        JTextField idField = new JTextField(target.getId(), 18);
        JComboBox<UseCaseNode.Kind> kindCombo =
                new JComboBox<>(UseCaseNode.Kind.values());
        ...
        panel.add(new JLabel(Messages.get("sketch.uc.dlg.id")));
        panel.add(idField);
        panel.add(new JLabel(Messages.get("sketch.uc.dlg.kind")));
        panel.add(kindCombo);
        panel.add(new JLabel(Messages.get("sketch.uc.dlg.label")));
        panel.add(labelField);

        int choice = JOptionPane.showConfirmDialog(parent, panel,
                Messages.get("sketch.uc.dlg.title"),
        ...
        target.setKind((UseCaseNode.Kind) kindCombo.getSelectedItem());
     ...
     */
    static boolean editRelation(Component parent, UseCaseRelation rel) {
        JComboBox<UseCaseRelation.Kind> kindCombo =
                new JComboBox<>(UseCaseRelation.Kind.values());
        kindCombo.setSelectedItem(rel.getKind());
        JTextField labelField = new JTextField(
                rel.getLabel() == null ? "" : rel.getLabel(), 18);
        JPanel panel = new JPanel(new GridLayout(2, 2, 6, 4));
        panel.add(new JLabel(Messages.get("sketch.uc.dlg.relKind")));
        panel.add(kindCombo);
        panel.add(new JLabel(Messages.get("sketch.uc.dlg.relLabel")));
        panel.add(labelField);
        int choice = JOptionPane.showConfirmDialog(parent, panel,
                Messages.get("sketch.uc.dlg.relTitle"),
                JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE);
        if (choice != JOptionPane.OK_OPTION) {
            return false;
        }
        rel.setKind((UseCaseRelation.Kind) kindCombo.getSelectedItem());
        String label = labelField.getText().trim();
