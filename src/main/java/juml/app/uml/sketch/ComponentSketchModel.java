...
/**
 * GUI デザイナーが編集するコンポーネント図モデル (コンポーネント/インターフェース群 + 関係群)。
 *
 * <p>PlantUML テキストとの相互変換は {@link ComponentSketchCodec} が担う。
 * このクラスは構造の保持と基本操作 (追加・削除・名前解決) のみ。</p>
 */
public final class ComponentSketchModel {

    private final List<ComponentNode> nodes = new ArrayList<>();
    private final List<ComponentRelation> relations = new ArrayList<>();
    ...
    /** id で要素を探す (無ければ null)。 */
    public ComponentNode findNode(String id) {
    ...
    public void removeNode(ComponentNode target) {
        nodes.remove(target);
        relations.removeIf(r -> r.touches(target.getId()));
    }

    /** id の変更に追随して関係の端点も付け替える。 */
    public void renameNode(ComponentNode target, String newId) {
        String old = target.getId();
        target.setId(newId);
        for (ComponentRelation r : relations) {
