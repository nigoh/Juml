...
/**
 * git 履歴の 2 時点間で、あるメソッドの<b>アクティビティ図</b>(if/loop/return の制御フロー)
 * を左右に並べて比較するモードレスダイアログ。「関数の中身がどう変わったか」を図で見る。
 *
 * <p>上部のコンボでメソッドを選び、左に旧・右に新のアクティビティ図を描く。変化した
 * アクションノードは図中で色付けする ({@link ActivityDiffHighlight})。シグネチャが同じで
 * 本体だけ変わった関数も検出できるよう、変更有無は生成した図テキストの差で判定する。</p>
 */
final class GitActivityCompareDialog extends JDialog {
    ...
    GitActivityCompareDialog(Window owner, GitRepoService svc, String relPath,
        ...
        super(owner, Messages.get("git.actcmp.title") + " - " + relPath,
    ...
    /** 選択メソッドの旧/新アクティビティ図を生成・色付けし、左右に描画する。 */
    private void renderMethod(Entry entry) {
        ...
        new SwingWorker<RenderedSvg[], Void>() {
            @Override protected RenderedSvg[] doInBackground() {
                String oldAct = activityOf(oldClasses, entry);
                String newAct = activityOf(newClasses, entry);
                String[] colored = ActivityDiffHighlight.colorize(oldAct, newAct);
    ...
    /** 対象メソッドが存在すればアクティビティ図 PlantUML を返す。無ければ空文字。 */
    private static String activityOf(List<JavaClassInfo> classes, Entry entry) {
        if (!hasMethod(classes, entry)) {
            return "";
        }
        PlantUmlActivityDiagram.Options opt = new PlantUmlActivityDiagram.Options();
        opt.includeLegend = false;
        return PlantUmlActivityDiagram.generate(
                classes, entry.className, entry.methodName, opt);
    }

    /** 変更のあるメソッドを先頭に並べたエントリ一覧を作る。 */
    private static List<Entry> buildEntries(List<JavaClassInfo> oldC,
                                            List<JavaClassInfo> newC) {
        ...
        for (String[] p : pairs) {
            Entry probe = new Entry(p[0], p[1], false);
            boolean diff = detect && !activityOf(oldC, probe).equals(activityOf(newC, probe));
    ...
    private static boolean hasMethod(List<JavaClassInfo> classes, Entry entry) {
        for (JavaClassInfo c : classes) {
            if (c.getSimpleName().equals(entry.className)) {
                for (JavaMethodInfo m : c.getMethods()) {
                    if (m.getName().equals(entry.methodName)) {
                        return true;
                    }
                }
            }
