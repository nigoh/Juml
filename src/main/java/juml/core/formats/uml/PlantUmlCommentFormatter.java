...
public final class PlantUmlCommentFormatter {
    ...
    /**
     * {@code "…"} で囲むラベルへ埋め込む文字列を安全な形へ整える。
     *
     * <p>引用符付きラベルは 2 通りに壊れる。{@code "} が入ると<b>ラベルが途中で閉じて</b>
     * PlantUML が構文エラーになり図が 1 枚も出ない。改行が入っても同じく途中で閉じる。
     * さらに creole/HTML タグは書式として解釈され、利用者が書いた文言が変形する
     * (実測: {@code Hello <b>world</b>} が 3 行に割れて world だけ太字になる)。</p>
     *
     * <p>この規則は Android 系の生成器で 5 回書かれていて、うち 3 つは
     * {@code "} を {@code '} に置くだけだった。同じ入力でも図種によって
     * <b>描けたり全損したり</b>する状態だったので、実装をここへ 1 本化する。</p>
     */
    public static String escapeQuotedLabel(String s) {
        if (s == null) {
            return "";
        }
        // まず引用符を潰し、空白 (改行・タブを含む) を 1 つに畳んでラベルを閉じさせない。
        // そのうえで共通エスケーパ (制御文字除去 / "<" のチルダエスケープ / "[[" の無害化 /
        // 図境界ディレクティブの分断) に通す。
        return escapeText(s.replace('"', '\'').replaceAll("\\s+", " ").trim());
    }
