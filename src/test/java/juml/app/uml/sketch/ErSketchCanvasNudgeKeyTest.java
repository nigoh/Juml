...
/**
 * bug-hunt round3 High 指摘の回帰テスト: {@link ErSketchCanvas} の矢印キー nudge を
 * 実 {@link KeyEvent} dispatch 経由で検証する。
 *
 * <p>{@code SketchNudge.deltaFor} は純関数として、{@code nudgeSelected} も直叩きで検証済み
 * だが、両者をつなぐ {@code handleKey} ラッパ ({@code editable && selected != null &&
 * !relationMode} ガード → {@code SketchNudge.deltaFor} → {@code nudgeSelected} →
 ...
 */
public class ErSketchCanvasNudgeKeyTest {
    ...
    private static ErSketchCanvas newCanvas(ErSketchCanvas.Listener listener) {
        return GuiActionRunner.execute(() -> new ErSketchCanvas(listener));
    }

    /** 登録済み KeyListener を EDT 上で直接起動する (オフスクリーンはフォーカス不可のため)。 */
    private static KeyEvent pressArrowKey(ErSketchCanvas canvas, int keyCode, boolean shift) {
    ...
    @Test
    public void arrowKey_movesSelectedEntity_rightAndDown_andConsumesEvent() {
        AtomicInteger edits = new AtomicInteger();
        ErSketchCanvas canvas = newCanvas(noopListener(edits));
        ErSketchModel.Entity a = new ErSketchModel.Entity("A", null, 40, 40);
    ...
    public void arrowKey_ignoredWhileRelationModeActive() {
        ...
        GuiActionRunner.execute(() -> {
            ...
            canvas.setRelationMode(true);
            canvas.setSelectedForTest(a);
        });

        pressArrowKey(canvas, KeyEvent.VK_RIGHT, false);

        assertEquals("リレーション追加モード中は矢印キーで動かないはず", 40, a.getX());
        assertEquals(0, edits.get());
    }
}
