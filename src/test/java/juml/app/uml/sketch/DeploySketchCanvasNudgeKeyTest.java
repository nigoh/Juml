...
/**
 * bug-hunt round3 High 指摘の回帰テスト: {@link DeploySketchCanvas} の矢印キー nudge を
 ...
 * linkMode == null} ガード → {@code SketchNudge.deltaFor} → {@code nudgeSelected} →
 ...
 */
public class DeploySketchCanvasNudgeKeyTest {
    ...
    private static DeploySketchCanvas newCanvas(DeploySketchCanvas.Listener listener) {
        return GuiActionRunner.execute(() -> new DeploySketchCanvas(listener));
    }

    /** 登録済み KeyListener を EDT 上で直接起動する (オフスクリーンはフォーカス不可のため)。 */
    private static KeyEvent pressArrowKey(DeploySketchCanvas canvas, int keyCode, boolean shift) {
    ...
    public void arrowKey_movesSelectedNode_rightAndDown_andConsumesEvent() {
        ...
        DeploySketchCanvas canvas = newCanvas(noopListener(edits));
    ...
    @Test
    public void arrowKey_ignoredWhileLinkModeActive() {
        AtomicInteger edits = new AtomicInteger();
        DeploySketchCanvas canvas = newCanvas(noopListener(edits));
        DeployNode a = new DeployNode(DeployNode.Kind.NODE, "A", null, 40, 40);
        DeploySketchModel model = new DeploySketchModel();
        ...
        GuiActionRunner.execute(() -> {
            canvas.setModel(model, true, List.of());
            // setLinkMode は selected をクリアするため、モード ON の後に強制選択する
        ...
        assertEquals("リンク追加モード中は矢印キーで動かないはず", 40, a.getX());
        assertEquals(0, edits.get());
    }
}
