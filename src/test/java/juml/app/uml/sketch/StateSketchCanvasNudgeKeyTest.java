...
/**
 * bug-hunt round3 High 指摘の回帰テスト: {@link StateSketchCanvas} の矢印キー nudge を
 ...
 * !transitionMode} ガード → {@code SketchNudge.deltaFor} → {@code nudgeSelected} →
 ...
 */
public class StateSketchCanvasNudgeKeyTest {
    ...
    private static StateSketchCanvas newCanvas(StateSketchCanvas.Listener listener) {
        return GuiActionRunner.execute(() -> new StateSketchCanvas(listener));
    }

    /** 登録済み KeyListener を EDT 上で直接起動する (オフスクリーンはフォーカス不可のため)。 */
    private static KeyEvent pressArrowKey(StateSketchCanvas canvas, int keyCode, boolean shift) {
    ...
    @Test
    public void arrowKey_movesSelectedState_rightAndDown_andConsumesEvent() {
        AtomicInteger edits = new AtomicInteger();
        StateSketchCanvas canvas = newCanvas(noopListener(edits));
    ...
    @Test
    public void arrowKey_ignoredWhileTransitionModeActive() {
        AtomicInteger edits = new AtomicInteger();
        StateSketchCanvas canvas = newCanvas(noopListener(edits));
        StateNode a = new StateNode("A", 40, 40);
        StateSketchModel model = new StateSketchModel();
        ...
        GuiActionRunner.execute(() -> {
            canvas.setModel(model, true, List.of());
            // setTransitionMode は selected をクリアするため、モード ON の後に強制選択する
            // (モードガード自体 (コピペ取り違え) を単体で検証するため)。
            canvas.setTransitionMode(true);
            canvas.setSelectedForTest(a);
        });

        pressArrowKey(canvas, KeyEvent.VK_RIGHT, false);

        assertEquals("遷移追加モード中は矢印キーで動かないはず", 40, a.getX());
        assertEquals(0, edits.get());
    }
}
