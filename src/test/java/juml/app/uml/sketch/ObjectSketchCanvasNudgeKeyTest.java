...
/**
 * bug-hunt round3 High 指摘の回帰テスト: {@link ObjectSketchCanvas} の矢印キー nudge を
 ...
 */
public class ObjectSketchCanvasNudgeKeyTest {
    ...
    private static ObjectSketchCanvas newCanvas(ObjectSketchCanvas.Listener listener) {
        return GuiActionRunner.execute(() -> new ObjectSketchCanvas(listener));
    }

    /** 登録済み KeyListener を EDT 上で直接起動する (オフスクリーンはフォーカス不可のため)。 */
    private static KeyEvent pressArrowKey(ObjectSketchCanvas canvas, int keyCode, boolean shift) {
    ...
    @Test
    public void arrowKey_movesSelectedObject_rightAndDown_andConsumesEvent() {
        AtomicInteger edits = new AtomicInteger();
        ObjectSketchCanvas canvas = newCanvas(noopListener(edits));
