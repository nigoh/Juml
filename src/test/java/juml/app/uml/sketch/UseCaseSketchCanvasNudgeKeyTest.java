...
/**
 * bug-hunt round3 High 指摘の回帰テスト: {@link UseCaseSketchCanvas} の矢印キー nudge を
 ...
 */
public class UseCaseSketchCanvasNudgeKeyTest {
    ...
    private static UseCaseSketchCanvas newCanvas(UseCaseSketchCanvas.Listener listener) {
        return GuiActionRunner.execute(() -> new UseCaseSketchCanvas(listener));
    }

    /** 登録済み KeyListener を EDT 上で直接起動する (オフスクリーンはフォーカス不可のため)。 */
    private static KeyEvent pressArrowKey(UseCaseSketchCanvas canvas, int keyCode, boolean shift) {
    ...
    public void arrowKey_movesSelectedNode_rightAndDown_andConsumesEvent() {
        ...
        UseCaseSketchCanvas canvas = newCanvas(noopListener(edits));
        UseCaseNode a = new UseCaseNode(UseCaseNode.Kind.USECASE, "A", null, 40, 40);
    ...
    public void arrowKey_ignoredWhileRelationModeActive() {
        ...
        GuiActionRunner.execute(() -> {
            ...
            canvas.setRelationMode(UseCaseRelation.Kind.ASSOCIATION);
            canvas.setSelectedForTest(a);
        });
