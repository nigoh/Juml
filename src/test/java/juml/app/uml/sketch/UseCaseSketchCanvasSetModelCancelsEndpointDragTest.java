...
/**
 * {@link UseCaseSketchCanvas} で端点ドラッグ中に {@code setModel} を呼ぶと、進行中の端点
 * ドラッグが中断されることを検証する ({@code UseCaseSketchCanvas.java:157} の
 ...
 * 飛ばないこと、で固定する。</p>
 */
public class UseCaseSketchCanvasSetModelCancelsEndpointDragTest {
    ...
    public void setModelDuringEndpointDrag_cancelsDragAndLaterReleaseDoesNotReattach() {
        ...
        UseCaseSketchModel newModel = new UseCaseSketchModel();
        UseCaseNode nodeD = new UseCaseNode(UseCaseNode.Kind.ACTOR, "D", null, 40, 100);
        UseCaseNode nodeE = new UseCaseNode(UseCaseNode.Kind.ACTOR, "E", null, 300, 100);
        newModel.getNodes().add(nodeD);
        newModel.getNodes().add(nodeE);
        GuiActionRunner.execute(() -> canvas.setModel(newModel, true, List.of()));

        // ドラッグ中断後に release しても、新モデルに孤立 reattach/modelEdited が起きないこと。
        Point target = centerOf(nodeE);
