...
/**
 * {@link ObjectSketchCanvas#nearestEndpointForTest} (端点ドラッグのヒットテストの核となる
 * 純関数) の境界値テスト。Swing/表示環境に依存しないため headless でも常に実行する。
 */
public class ObjectSketchCanvasEndpointGeometryTest {
    ...
    public void pickAOverB_whenCloserToA() {
        ...
        assertEquals(0, ObjectSketchCanvas.nearestEndpointForTest(p, A, B, 8.0));
    }

    @Test
    public void pickBOverA_whenCloserToB() {
        Point p = new Point(199, 100);
        assertEquals(1, ObjectSketchCanvas.nearestEndpointForTest(p, A, B, 8.0));
    }
    ...
    public void picksNearerSide_whenBothWithinRadius() {
        ...
        assertEquals(0, ObjectSketchCanvas.nearestEndpointForTest(p, A, B, 200.0));
    }
}
