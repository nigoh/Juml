...
/**
 * {@link UseCaseSketchCanvas} の端点ハンドルしきい値 (ヒット半径のズーム対応) の配線検証。
 *
 ...
 * 画面上約 8px 以内かを判定する。純関数自体は {@link UseCaseSketchCanvasReattachTest} で
 ...
 * 作法を UseCase 図へ適用する。UseCase は {@code setZoomForTest} を持たないため、縮小は
 * 実際の Ctrl+ホイールディスパッチ ({@link SketchViewport#onWheel}) で行う。</p>
 */
public class UseCaseSketchCanvasEndpointClickThresholdTest {
    ...
    private UseCaseSketchCanvas.Listener listener() {
    ...
    private UseCaseSketchCanvas newCanvas() {
        return GuiActionRunner.execute(() -> new UseCaseSketchCanvas(listener()));
    }

    /** A(40,100) --> B(300,100) の関係 1 本と、離れた付替え先候補 C(560,100) を持つモデル。 */
    private static UseCaseSketchModel sampleModel() {
        UseCaseSketchModel model = new UseCaseSketchModel();
        model.getNodes().add(new UseCaseNode(UseCaseNode.Kind.ACTOR, "A", null, 40, 100));
        model.getNodes().add(new UseCaseNode(UseCaseNode.Kind.ACTOR, "B", null, 300, 100));
        model.getNodes().add(new UseCaseNode(UseCaseNode.Kind.ACTOR, "C", null, 560, 100));
        model.getRelations().add(
                new UseCaseRelation("A", UseCaseRelation.Kind.ASSOCIATION, "B", null));
        return model;
    }

    private void dispatch(UseCaseSketchCanvas canvas, int id, int modifiersEx, Point p, int button) {
    ...
    private static Point rightMid(UseCaseSketchCanvas canvas, UseCaseNode n) {
        Rectangle r = GuiActionRunner.execute(() -> canvas.boundsOf(n));
        return new Point(r.x + r.width, r.y + r.height / 2);
    }

    private static Point centerOf(UseCaseSketchCanvas canvas, UseCaseNode n) {
        Rectangle r = GuiActionRunner.execute(() -> canvas.boundsOf(n));
        return new Point(r.x + r.width / 2, r.y + r.height / 2);
    }

    /** Ctrl+ホイールを繰り返しディスパッチし、実操作の経路で {@link SketchViewport#MIN_ZOOM} まで縮小する。 */
    private static void zoomOutToMinViaWheel(UseCaseSketchCanvas canvas) {
    ...
    public void clickWithoutMoving_doesNotReattachOrFireModelEdited() {
        UseCaseSketchCanvas canvas = newCanvas();
        UseCaseSketchModel model = sampleModel();
        UseCaseRelation rel = model.getRelations().get(0);
        UseCaseNode a = model.getNodes().get(0);
        GuiActionRunner.execute(() -> {
    ...
    public void pressWithinHandleRadius_atDefaultZoom_startsEndpointDragAndReattaches() {
        ...
        UseCaseRelation rel = model.getRelations().get(0);
        UseCaseNode a = model.getNodes().get(0);
        UseCaseNode c = model.getNodes().get(2);
        GuiActionRunner.execute(() -> {
            canvas.setModel(model, true, List.of());
            canvas.setSize(800, 400);
        });
