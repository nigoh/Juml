...
/**
 * bug-hunt round10 の回帰テスト: ER 図デザイナーでリレーション追加モード中に始点エンティティを
 * 右クリック削除すると、修正前は {@code relationSource} が削除済みインスタンスを
 * 指したまま残り、次にクリックしたエンティティとの間に削除済みエンティティを端点に持つ
 * 宙吊りのリレーションが生成されていた (往復 PlantUML にも宣言のない幽霊エンティティが復活する)。
 *
 * <p>{@link ErSketchCanvas} にはリレーションクリック用のテストシームが無いため、実際の
 * 左クリック ({@link MouseEvent#MOUSE_PRESSED}) をエンティティ中心へディスパッチして
 * {@code handleRelationClick} を通す (プロダクションの導線をそのまま踏む)。</p>
 */
public class ErSketchCanvasDeleteClearsPendingSourceTest {
    ...
    private static ErSketchCanvas.Listener noopListener(AtomicInteger edits) {
    ...
    private static void leftClick(ErSketchCanvas canvas, Point p) {
    ...
    public void deletingRelationSourceViaRightClick_clearsPendingSource() {
        ...
        ErSketchModel.Entity a = new ErSketchModel.Entity("A", null, 40, 100);
        ErSketchModel.Entity b = new ErSketchModel.Entity("B", null, 300, 100);
        ErSketchModel.Entity c = new ErSketchModel.Entity("C", null, 300, 300);
        model.getEntities().add(a);
        model.getEntities().add(b);
        model.getEntities().add(c);

        JFrame[] frameHolder = new JFrame[1];
        ErSketchCanvas canvas = GuiActionRunner.execute(() -> {
            ErSketchCanvas cv = new ErSketchCanvas(noopListener(edits));
        ...
        try {
            ...
            JMenuItem delete = findMenuItem(popup, Messages.get("sketch.er.menu.delete"));
            ...
            assertEquals("A が削除され B, C のみ残るはず", 2, model.getEntities().size());

            // 削除済み A が pending source のまま残っていれば、次に B をクリックした瞬間に
            // A に接続する宙吊りリレーションが完成してしまう (round10 の修正前バグ)。
            ...
            assertTrue("削除済み A を端点に持つ宙吊りリレーションが生成されないはず",
                    model.getRelations().isEmpty());

            // B が新しい始点として正しく確定していることを、C クリックでのリレーション完成で確認する。
            ...
            assertEquals("B->C のリレーションが 1 本だけ生成されるはず", 1, model.getRelations().size());
            ErSketchModel.Relation created = model.getRelations().get(0);
            assertEquals("B", created.getLeft());
            assertEquals("C", created.getRight());

            String puml = GuiActionRunner.execute(() -> ErSketchCodec.toPuml(model));
            assertFalse("再生成 PlantUML に削除済み A への参照 (幽霊エンティティ) が出ないはず: " + puml,
                    puml.contains("A"));
        } finally {
            GuiActionRunner.execute(() -> {
