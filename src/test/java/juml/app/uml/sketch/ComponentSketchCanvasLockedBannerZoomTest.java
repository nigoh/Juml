...
/**
 * {@link ComponentSketchCanvas} のロックバナー (未対応構文で編集を無効化したときの警告帯) が、
 ...
 * 常にキャンバス幅いっぱいに跨る。</p>
 */
public class ComponentSketchCanvasLockedBannerZoomTest {
    ...
    private ComponentSketchCanvas.Listener noopListener() {
    ...
    public void lockedBanner_spansCanvasWidthRegardlessOfZoom() {
        ComponentSketchCanvas canvas =
                GuiActionRunner.execute(() -> new ComponentSketchCanvas(noopListener()));
        ComponentSketchModel model = new ComponentSketchModel();
        model.getNodes().add(new ComponentNode(ComponentNode.Kind.COMPONENT, "A", null, 40, 40));

        int width = 800;
        int height = 600;
        GuiActionRunner.execute(() -> {
            // interface/component 以外の未対応構文 1 行を与えてロック状態 (editable=false) にする。
