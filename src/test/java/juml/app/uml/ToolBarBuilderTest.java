...
public class ToolBarBuilderTest {
    ...
    /** ツールチップ補足は先頭の区切りスペースを保持し、補足の無い図種は空文字を返すこと。 */
    @Test
    public void tooltipExtra_keepsLeadingSpaceAndEmptyForPlainKinds() {
        Messages.setLanguage("en");
        assertTrue(ToolBarBuilder.tooltipExtra(DiagramKind.SEQUENCE).startsWith(" "));
        assertEquals("", ToolBarBuilder.tooltipExtra(DiagramKind.CLASS));
