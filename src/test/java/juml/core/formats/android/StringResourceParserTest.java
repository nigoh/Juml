...
public class StringResourceParserTest {
    ...
    @Test
    public void testXliffPlaceholderNormalizedToToken() {
        // <xliff:g id="name"> は {name} トークンに正規化し、引数の意味を残す。
        String xml = "<resources xmlns:xliff=\"urn:oasis:names:tc:xliff:document:1.2\">\n"
                + "  <string name=\"welcome\">Hello <xliff:g id=\"name\" example=\"Sam\">"
                + "%1$s</xliff:g>!</string>\n"
                + "  <string name=\"count\"><xliff:g id=\"n\">%1$d</xliff:g> items</string>\n"
                + "</resources>";
        AndroidStringResources res = StringResourceParser.parse(xml);
        assertEquals("Hello {name}!", res.getString("welcome"));
        assertEquals("{n} items", res.getString("count"));
    }
