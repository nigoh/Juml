...
public class InsightsAnalyzerTest {
    ...
    @Test
    public void testOverloadedDeadMethodIsListedOnce() {
        // 同名オーバーロード (setTitle(String)/setTitle(int)) は名前単位で集約され、
        // dead code 候補に重複して並ばないこと。
        String bar = "package com.foo;\n"
                + "public class Bar {\n"
                ...
                + "  public void touched() {}\n"
                + "}\n";
        // Bar 自体は参照させ (CLASS 候補化を避ける)、setTitle だけ未使用にする
        String use = "package com.foo;\n"
                + "public class UseSite {\n"
                + "  void run() { new Bar().touched(); }\n"
                + "}\n";
        InsightsModel model = analyze(parse(bar, use));
        long count = model.getDeadCodeCandidates().stream()
                .filter(d -> d.getKind() == InsightsModel.DeadCodeCandidate.Kind.METHOD
                        && "com.foo.Bar.setTitle".equals(d.getSymbol()))
                .count();
        assertEquals("overloaded dead method must appear exactly once", 1, count);
    }
