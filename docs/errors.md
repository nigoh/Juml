...
- **概要**: PlantUML 共有 URL の生成・コピーに失敗しました
- **対処**: 代わりに PlantUML テキストをコピーして PlantUML サーバーへ手動で貼り付けてください。クリップボードが使えない場合は、占有している他のアプリを閉じて再試行してください。
...
- **概要**: エディタ下書き (自動保存) の読み書きに失敗しました
- **対処**: ディスクの空き容量と Juml 設定フォルダの書き込み権限を確認してください。自動保存なしで編集は継続できます。Ctrl+S で手動保存してください。
- **Summary**: Failed to read or write the editor draft (autosave)
- **Remedy**: Check free disk space and write permission of the Juml settings folder. Editing continues without autosave; save your work with Ctrl+S.
...
- **概要**: コマンドラインオプションに必要な値がありません
- **対処**: オプションの後に値を指定してください（例: -o output.svg）。--help も参照してください。
- **Summary**: Missing value for a command-line option
- **Remedy**: Provide a value after the option (for example: -o output.svg), or run with --help.
